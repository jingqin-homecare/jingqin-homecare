...
const CACHE_NAME = 'jingqin-homecare-v1';  // 請填入你的GitHub repo名稱，例如 'jingqin-journal-v1'
const INDEX_URL = '/jingqin-homecare/index.html';  // 請填入你的GitHub repo名稱
...
self.addEventListener('fetch', event => {
  ...
  const isAppPage =
    ...
    url.pathname === '/jingqin-homecare/' ||
    url.pathname === '/jingqin-homecare/index.html' ||
    url.pathname === '/jingqin-homecare/admin.html';
      ...
      fetch(event.request, { cache: 'no-store' })  // no-store：強制抓最新，不吃瀏覽器快取
        ...
        .catch(() => {
          return caches.match(INDEX_URL)
            .then(cached => cached || caches.match('/jingqin-homecare/'));
        })
    );
  }
});
